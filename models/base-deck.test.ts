...
describe("shuffle", () => {
  ...
  it("randomizes the order of the cards", () => {
    for (let i = 0; i < shuffledDeck.cards.length; i++) {
      expect(deck.cards[i]?.toString).not.toBe(
        shuffledDeck.cards[i]?.toString()
      );
    }
  });
  ...
  it("does not add or remove any cards", () => {
    ...
    shuffledDeck.cards.forEach((shuffledCard) => {
      expect(deck.cards.map((card) => card.toString())).toContain(
        shuffledCard.toString()
      );
    });

    deck.cards.forEach((card) => {
      expect(shuffledDeck.cards.map((card) => card.toString())).toContain(
        card.toString()
      );
    });
  });
...
test("size returns the length of the list of cards", () => {
  const deck = new BaseDeck();
  expect(deck.size).toBe(0);

  deck.insert("random", new Card("10", "club"));
  ...
  describe("random", () => {
    it("inserts the card at a random position", () => {
      ...
      const index = deck.cards.findIndex(
        (card) => card.toString() === drawnCard.toString()
      );
